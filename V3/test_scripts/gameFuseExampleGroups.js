    ...
    async run() {
        await Test.performTestLogic(this, async () => {
            ...
            await Test.describe('GET ATTRIBUTES', async () => {
                ...
                let options = {name: 'My Group 6', groupType: "multiplayer_party", canAutoJoin: false, isInviteOnly: false, maxGroupSize: 20}
                await Test.test('GameFuseGroup.create(options)', async () => {
                    await GameFuseGroup.create(options, () => console.log('group1 created'));
                })
                ...
                await Test.test('group.requestToJoin', async () => {
                    await group.join(() => console.log('user2 joined to join group1'));
                })


                await GameFuse.signIn(this.user1.getTestEmail(), 'password', () => console.log('user1 signed in'));
                await Test.test('group.createAttribute', async () => {
                    await group.createAttribute({ key: 'theme', value: 'dark', others_can_edit: false }, () => {
                        console.log('attribute created');
                    });
                });
                await Test.test('group.createAttribute', async () => {
                    await group.createAttribute({ key: 'size', value: 'large', others_can_edit: false }, () => {
                        console.log('attribute created');
                    });
                });
                ...
                await Test.test('group.getAttributes', async () => {
                    ...
                    Test.expect(group.attributes.length).toEqual(2, 'There should be at least one attribute fetched.');
                    Test.expect(group.attributes.some(attr => attr.key === 'theme')).toEqual(true, 'The fetched attributes should include the "theme" key.');
                    Test.expect(group.attributes.some(attr => attr.key === 'size')).toEqual(true, 'The fetched attributes should include the "size" key.');

                });
            });

            // 1) User succeeds in adding attribute when group admins_only_can_create_attributes==true because they are an admin
            await Test.describe('Admins-only: Admin can add attribute', async () => {
                console.log('Admins-only: Admin can add attribute');
                
                await GameFuse.signIn(this.user1.getTestEmail(), 'password', () => console.log('user1 signed in'));
                
                let options = { name: 'Admins Only Group', admins_only_can_create_attributes: true, maxGroupSize: 10 };
                await Test.test('GameFuseGroup.create(options)', async () => {
                ...
                await Test.test('group.createAttribute (success case)', async () => {
                    await group.createAttribute({ key: 'color', value: 'red', others_can_edit: false }, () => {
                        console.log('attribute created by admin');
                    });
                });

                await group.getAttributes(() => console.log('attributes fetched'));
                let colorAttribute = group.attributes.find(attr => attr.key === 'color');
                Test.expect(colorAttribute.value).toEqual('red', 'The attribute should be successfully created by an admin.');
            });

            await Test.describe('Admins-only: Non-admin cannot add attribute', async () => {
                console.log('Admins-only: Non-admin cannot add attribute');
            
                // Step 1: Sign in as user1 and create a new group with admins_only_can_create_attributes set to true
                await GameFuse.signIn(this.user1.getTestEmail(), 'password', () => console.log('user1 signed in'));
                let options = {
                    name: 'Admins Only Test Group',
                    admins_only_can_create_attributes: true,
                    maxGroupSize: 10,
                    canAutoJoin: true, 
                    isInviteOnly: false
                };
                await GameFuseGroup.create(options, () => console.log('Group created by admin'));
            
                // Retrieve the created group
                let group = currentUser().getGroups()[0];

                // Step 2: Add user2 as a non-admin member of the group
                await GameFuse.signIn(this.user2.getTestEmail(), 'password', () => console.log('user2 signed in'));
                await currentUser().downloadFullData(() => console.log('downloaded game rounds for user1'), []);
                await group.join(() => console.log('user2 joined the group'));
            
                // Step 3: Attempt to add an attribute as user2
                await Test.test('group.createAttribute (failure case)', async () => {
                    try {
                        await group.createAttribute({ key: 'color', value: 'blue', others_can_edit: false }, () => {
                            console.log('Attribute creation attempt by non-admin');
                        });
                    } catch (error) {
                        console.error('Error during attribute creation attempt by non-admin:', error.message);
                    }
                });
            
                // Step 4: Verify that the attribute was not created
                await group.downloadFullData(() => console.log('downloaded full data for the group'));
                await group.getAttributes(() => console.log('Attributes fetched'));
                let colorAttribute = group.attributes.find(attr => attr.key === 'color');

                Test.expect(colorAttribute).toEqual(undefined, 'The attribute should not be created by a non-admin.');
            });

            // 3) User succeeds in adding attribute when group admins_only_can_create_attributes==false when they are an admin
            await Test.describe('All can add: Admin can add attribute', async () => {
                console.log('All can add: Admin can add attribute');

                await GameFuse.signIn(this.user1.getTestEmail(), 'password', () => console.log('user1 signed in'));
                let group = currentUser().getGroups()[0];
                group.admins_only_can_create_attributes = false;
                ...
                await group.getAttributes(() => console.log('attributes fetched'));
                let sizeAttribute = group.attributes.find(attr => attr.key === 'size');
                Test.expect(sizeAttribute.value).toEqual('large', 'The attribute should be successfully created by an admin.');
            });

            // 4) User succeeds in adding attribute when group admins_only_can_create_attributes==false when they are not an admin
            await Test.describe('All can add: Non-admin can add attribute', async () => {
                console.log('All can add: Non-admin can add attribute');
                ...
                await Test.test('group.createAttribute (success case)', async () => {
                    await group.createAttribute({ key: 'theme', value: 'dark', others_can_edit: false }, () => {
                        console.log('attribute created by non-admin');
                    });
                });

                await group.getAttributes(() => console.log('attributes fetched'));
                let themeAttribute = group.attributes.find(attr => attr.key === 'theme');
                Test.expect(themeAttribute.value).toEqual('dark', 'The attribute should be successfully created by a non-admin.');
            });

            // 5) User succeeds in modifying attribute when group admins_only_can_create_attributes==true because they are an admin
            await Test.describe('Admins-only: Admin can modify attribute', async () => {
                console.log('Admins-only: Admin can modify attribute');
                ...
                await Test.test('group.modifyAttribute (success case)', async () => {
                    await group.modifyAttribute('theme', 'light', () => {
                        console.log('attribute modified by admin');
                    });
                });

                await group.getAttributes(() => console.log('attributes fetched'));
                let themeAttribute = group.attributes.find(attr => attr.key === 'theme');
                Test.expect(themeAttribute.value).toEqual('light', 'The attribute should be successfully modified by an admin.');
            });

            // 6) User fails in modifying attribute when group admins_only_can_create_attributes==true because they are not an admin
            await Test.describe('Admins-only: Non-admin cannot modify attribute', async () => {
                console.log('Admins-only: Non-admin cannot modify attribute');
                ...
                await group.createAttribute({ key: 'theme', value: 'light', others_can_edit: false }, () => {
                    console.log('attribute created by non-admin');
                });

                await GameFuse.signIn(this.user1.getTestEmail(), 'password', () => console.log('user1 signed in'));
                await group.join(() => console.log('user1 joined the group'));

                await Test.test('group.modifyAttribute (failure case)', async () => {
                    await group.modifyAttribute('theme', 'blue', () => {
                        console.log('unauthorized modification attempt');
                    });
                });
                ...
                await group.getAttributes(() => console.log('attributes fetched'));
                let themeAttribute = group.attributes.find(attr => attr.key === 'theme');
                Test.expect(themeAttribute.value).toEqual('light', 'The attribute should not be modified by a non-admin.');
            });

            // 7) User succeeds in modifying attribute when group admins_only_can_create_attributes==false when they are an admin
            await Test.describe('All can modify: Admin can modify attribute', async () => {
                console.log('All can modify: Admin can modify attribute');

                await GameFuse.signIn(this.user1.getTestEmail(), 'password', () => console.log('user1 signed in'));
                
                let options = {
                    name: 'Admins Only Test Group 7',
                    admins_only_can_create_attributes: false,
                    maxGroupSize: 10,
                    canAutoJoin: true, 
                    isInviteOnly: false
                };
                ...
                await Test.test('group.modifyAttribute (success case)', async () => {
                    await group.modifyAttribute('theme', 'green', () => {
                        console.log('attribute modified by admin');
                    });
                });
                ...
                await group.getAttributes(() => console.log('attributes fetched'));
                let themeAttribute = group.attributes.find(attr => attr.key === 'theme');
                Test.expect(themeAttribute.value).toEqual('green', 'The attribute should be successfully modified by an admin.');
            });

            // 8) User succeeds in modifying attribute when group admins_only_can_create_attributes==false when they are not an admin
            await Test.describe('All can modify: Non-admin can modify attribute', async () => {
                console.log('All can modify: Non-admin can modify attribute');

                await GameFuse.signIn(this.user2.getTestEmail(), 'password', () => console.log('user2 signed in'));
                let options = {
                    name: 'Admins Only Test Group 8',
                    admins_only_can_create_attributes: false,
                    maxGroupSize: 10,
                    canAutoJoin: true, 
                    isInviteOnly: false
                ...
                await group.createAttribute({ key: 'theme', value: 'light', others_can_edit: true }, () => {
                    console.log('attribute created by non-admin');
                });
                ...
                await Test.test('group.modifyAttribute (success case)', async () => {
                    await group.modifyAttribute('theme', 'yellow', () => {
                        console.log('attribute modified by non-admin');
                    });
                });

                await group.getAttributes(() => console.log('attributes fetched'));
                let themeAttribute = group.attributes.find(attr => attr.key === 'theme');
                Test.expect(themeAttribute.value).toEqual('yellow', 'The attribute should be successfully modified by a non-admin.');
            });
        });
    }
}
