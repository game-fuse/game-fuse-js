class GameFuseExampleMessages {
    ...
    async run() {
        await Test.performTestLogic(this, async () => {
            ...
            await Test.test('Expect the chat data for user2 to be accurate', async () => {
            ...
            await Test.test('Mark a message as read', async () => {
                await GameFuse.signIn(this.user2.getTestEmail(), 'password', () => {});
                let chat = currentUser().getDirectChats()[0];
                let messages = chat.getMessages();

                let messageToMarkRead = messages[0];
                console.log(messageToMarkRead)
                Test.expect(messageToMarkRead.getIsRead()).toEqual(false, 'The message should initially be unread');

                await messageToMarkRead.markAsRead(() => {
                    console.log('Marked the message as read');
                });

                Test.expect(messageToMarkRead.getIsRead()).toEqual(true, 'The message should now be marked as read');
            });
